...
Function BeforeFileOpenHook2(refnum, filename, symPath, type, creator, kind)

	Variable	refnum, kind
	String 		filename, symPath, type, creator

	//fileType(filename, symPath, refnum)		// path is the symbolic path!

	//return 1	// don't let igor open the file 
//	HDF5TEST(sympath, filename)
	return fileType(filename, symPath, refnum)
print filename
print sympath
print type
End
...
Function fileType2(FileName, symPath, refnum)
	...
	// find the extension: characters after last dot (updated 11/13/08 for compatibility with multiple dots in filenames)
	from = -1
	do
		from = to
		to = strsearch(FileName,".",from+1)
		if (to == -1)
			break
		endif
	while (1)
	
	ext = FileName[from+1, strlen(fileName)-1]
	
	//ext = FileName[strsearch(FileName,".",0)+1, strlen(fileName)-1]
	
	//-------------------- Croissant - files: ---------------------------------
	
	if (string_List_check2(ext,"nxs",";"))	// NeXus ARPES extension

		print "NeXus File detected, please wait while loading"
		HDF5test(symPath, FileName)
...
Function string_List_check2(guess,List,separator)
